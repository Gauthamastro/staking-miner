...
pub(crate) static mut RUNTIME: AnyRuntime = AnyRuntime::Polkadex;
...
macro_rules! construct_runtime_prelude {
	($runtime:ident) => { paste::paste! {
		pub(crate) mod [<$runtime _runtime_exports>] {
			...
			pub(crate) use polkadex_runtime::Call as RuntimeCall;
			...
			mod private {
				...
				pub(crate) fn [<create_uxt_ $runtime>](
					...
					let local_call = pallet_election_provider_multi_phase::Call::<Runtime>::submit { raw_solution: Box::new(raw_solution) };
					let call: RuntimeCall = <pallet_election_provider_multi_phase::Call<Runtime> as std::convert::TryInto<RuntimeCall>>::try_into(local_call)
...
fn signed_ext_builder_polkadex(
	nonce: Index,
	tip: Balance,
	era: sp_runtime::generic::Era,
) -> polkadex_runtime_exports::SignedExtra {
	use polkadex_runtime_exports::Runtime;
	(
...
construct_runtime_prelude!(polkadex);
...
macro_rules! any_runtime {
	($($code:tt)*) => {
		unsafe {
			match $crate::RUNTIME {
				$crate::AnyRuntime::Polkadex => {
					#[allow(unused)]
					use $crate::polkadex_runtime_exports::*;
					$($code)*
				},
			}
		}
...
async fn main() {
	...
	match chain.to_lowercase().as_str() {
		"Polkadex Main Network" => {
			...
			sub_tokens::dynamic::set_name("PDEX");
